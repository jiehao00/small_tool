    ...
    sections = [
        ("功能一：填充数据（Excel 内容填充）", [
            ...
            ("操作步骤",
             ...
             "3. 点击匹配列旁的 🏷️ 选列按钮，在弹出的列名列表中勾选两表的匹配列\n"
             "   （如 A[编号] ↔ B[ID]），Ctrl+点击多选，按选择顺序配对\n"
             "4. 同样点击填充列旁的 🏷️ 选列按钮，勾选 A 表哪列要填、B 表哪列是数据来源\n"
             "5. 设置输出文件路径，点击「开始转换」"),
            ("匹配规则",
...
"2. 点击 🏷️ 选列按钮，在弹出的列名列表中 Ctrl+点击勾选关联列\n"
            "   如 A🏷️ → 勾选 [编号, 姓名]   B🏷️ → 勾选 [ID, Name]\n"
             "   选中的列名自动填入文本框，按选择顺序一一配对\n"
             "3. 核对两侧列数一致后，设置输出路径，点击「开始对比」"),
            ("输出结果",
             "一个 Excel 文件包含三个 Sheet：\n"
        ...
        ("功能三：财务对账（汇总表 vs 明细累加对比）", [
            ...
            ("操作步骤",
             ...
             "2. 点击主键列旁的 🏷️ 选列按钮，分别选择两表的匹配列\n"
             "   如 汇总[发票号] ↔ 明细[发票号]，Ctrl+点击可多选复合主键\n"
             "3. 点击对比列旁的 🏷️ 选列按钮，分别选择两表要核对的数值列\n"
